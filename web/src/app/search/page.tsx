...
export default function SmartSearch() {
  ...
  const executeSearch = async (searchTerm: string) => {
    ...
    try {
      docs = JSON.parse(localStorage.getItem(`arkivex_user_docs_${userEmail}`) || "[]");
    } catch (e) {}

    // Fetch from backend API if locally accessible
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className="px-3 py-1 text-xs font-extrabold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">
                    {doc.storageProvider || "Supabase Cloud Storage"}
                  </span>
                </div>
